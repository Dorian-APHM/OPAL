...
def global_search(
    ...
    cohort_q = db.query(Cohort).filter(
        Cohort.name.ilike(f"%{search_term}%")
    )
    if cdm_name:
                    ...
                    with conn.cursor(cursor_factory=DictCursor) as cur:
                        if search_term.isdigit():
                            ...
                            sql = f"""
                                ...
                                LIMIT %s
                            """
                            cur.execute(sql, (f"%{search_term}%", f"%{search_term}%", limit))
                        results["concepts"] = [
                            {
                        ...
                        try:
                            ...
                            for domain_name, cfg in DOMAIN_CONFIG.items():
                                ...
                                query_params = [domain_name, f"%{search_term}%"]
                                if source_name_col:
                            ...
                            if union_parts:
                                sv_sql = " UNION ALL ".join(union_parts)
                                sv_sql = f"SELECT * FROM ({sv_sql}) sub ORDER BY n_records DESC LIMIT %s"
    ...
    sq_q = db.query(SavedQuery).filter(SavedQuery.name.ilike(f"%{search_term}%"))
    if cdm_name:
    ...
    map_q = db.query(MappingDecision).filter(
        MappingDecision.source_value.ilike(f"%{search_term}%")
    )
    if cdm_name:
