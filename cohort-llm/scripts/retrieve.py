...
def expand_label(label: str, model: str = OLLAMA_MODEL) -> str:
    r = requests.post(OLLAMA_URL, json={
        "model": model,
        "system": EXPAND_SYSTEM,
        "prompt": f"Terme: {label}\nRéponse:",
        "stream": False,
        "options": {"temperature": 0, "seed": 1, "num_predict": 60, "top_p": 1.0},
    }, timeout=60).json()
    txt = r.get("response", "").strip().split("\n")[0].strip()
