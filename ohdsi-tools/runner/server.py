...
def _terminate_job_process(proc: subprocess.Popen, grace: float = 10.0) -> None:
    """Stop a job's process group: SIGTERM first, then SIGKILL if it ignores it.

    SIGTERM is sent synchronously; a daemon thread escalates to SIGKILL after
    `grace` seconds if the group (R + its JVM children) is still alive, so a
    wedged tool can't survive a cancel. Reaping stays with the job thread
    (proc.wait()); here we only poll, never wait, to avoid double-reaping."""
    try:
    ...
    def _escalate() -> None:
        ...
        while time.monotonic() < deadline:
            if proc.poll() is not None:
                return  # exited on SIGTERM; the job thread will reap it
            time.sleep(0.5)
        try:
